...
let FriendsListComponent = class FriendsListComponent {
    constructor(friendServ) {
        this.friendServ = friendServ;
        this.isFriend = false;
    }
    ngOnInit() {
        const userId = this.user.uid;
        const currentUserId = firebase__WEBPACK_IMPORTED_MODULE_4__["auth"]().currentUser.uid;
...
let FriendsService = class FriendsService {
    constructor(db, afAuth, afs) {
        ...
        //     return { id, ...data };
        //   });;
        this.users = this.usersCollection.snapshotChanges().pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_6__["map"])(actions => actions.map(a => {
            const data = a.payload.doc.data();
            const id = a.payload.doc.id;
            return Object.assign({ id }, data);
        })));
    }
    getTotalFriends(userId) {
        // Used to build the follower count
        return this.db.object(`friends/${userId}`);
    }
    areFriends(followerId) {
...
let FriendsComponent = class FriendsComponent {
    ...
    addFriend(friendToAdd) {
        ...
        this.notAdded = false;
        this.remove = true;
        this.friendsDocument.update({ friendsList: firebase__WEBPACK_IMPORTED_MODULE_7__["firestore"].FieldValue.arrayUnion({ [friendToAdd]: true }) });
        // adds your id to 'friends' friend list
        this.yourFriendsDocument = this.afs.doc(`friends/` + friendToAdd);
        this.yourFriendsDocument.update({ friendsList: firebase__WEBPACK_IMPORTED_MODULE_7__["firestore"].FieldValue.arrayUnion({ [this.id]: true }) });
        console.log(friendToAdd + 'this is you');
    }
    removeFriend(friendToRemove) {
        this.notAdded = true;
        this.remove = false;
        this.friendsDocument.update({ friendsList: firebase__WEBPACK_IMPORTED_MODULE_7__["firestore"].FieldValue.arrayRemove({ [friendToRemove]: true }) });
        // removes you from their friendslist
        this.yourFriendsDocument = this.afs.doc(`friends/` + friendToRemove);
        this.yourFriendsDocument.update({ friendsList: firebase__WEBPACK_IMPORTED_MODULE_7__["firestore"].FieldValue.arrayRemove({ [this.id]: true }) });
...
let UserFriendListComponent = class UserFriendListComponent {
...
UserFriendListComponent.ctorParameters = () => [
    { type: _auth_service__WEBPACK_IMPORTED_MODULE_3__["AuthService"] },
    { type: _angular_fire_firestore__WEBPACK_IMPORTED_MODULE_2__["AngularFirestore"] }
];
...
let UserMeetingListComponent = class UserMeetingListComponent {
    constructor(auth, afs) {
        this.auth = auth;
        this.afs = afs;
        this.id = firebase__WEBPACK_IMPORTED_MODULE_4__["auth"]().currentUser.uid;
...
UserMeetingListComponent.ctorParameters = () => [
    { type: _auth_service__WEBPACK_IMPORTED_MODULE_2__["AuthService"] },
    { type: _angular_fire_firestore__WEBPACK_IMPORTED_MODULE_3__["AngularFirestore"] }
];
